describe('Help documentation', () => {
  ...
  context('Player role', () => {
    ...
    it('shows player-accessible entries on /help', () => {
      cy.visit('/help')
      cy.contains('h2', 'Events').scrollIntoView().should('be.visible')
      cy.contains('h2', 'My Characters').scrollIntoView().should('be.visible')
      cy.contains('h2', 'Rulebook').scrollIntoView().should('be.visible')
    })
    ...
    it('opens the help modal with page-specific content when help button is clicked', () => {
      cy.visit('/events')
      cy.get('[data-testid="help-button"]').first().click()
