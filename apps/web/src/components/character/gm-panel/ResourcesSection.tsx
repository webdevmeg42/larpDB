        ...
        {consumables.map((c, i) => (
          <div key={i} className="flex gap-2 items-center">
            ...
            <Input
              ...
              className="w-20"
            />
            <button type="button" aria-label="Remove consumable" onClick={() => setConsumables(p => p.filter((_, j) => j !== i))} className="text-muted-foreground hover:text-destructive text-xs">✕</button>
          </div>
        ))}
          ...
          <div key={i} className="rounded border p-3 space-y-2">
            <div className="flex gap-2 items-center">
              <Input
                ...
                className="flex-1"
              />
              <button type="button" aria-label="Remove inventory item" onClick={() => setInventory(p => p.filter((_, j) => j !== i))} className="text-muted-foreground hover:text-destructive text-xs">✕</button>
            </div>
            <Input
        ...
        {crafting.map((m, i) => (
          <div key={i} className="flex gap-2 items-center">
            ...
            <Input
              ...
              className="w-20"
            />
            <button type="button" aria-label="Remove crafting material" onClick={() => setCrafting(p => p.filter((_, j) => j !== i))} className="text-muted-foreground hover:text-destructive text-xs">✕</button>
          </div>
        ))}
