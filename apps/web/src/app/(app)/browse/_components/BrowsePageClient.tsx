...
export function BrowsePageClient({ initialGames }: Props) {
  ...
  useEffect(() => {
    // Background refresh keeps data current; client-side fetch also satisfies test intercepts.
    api.get<{ items: BrowseGame[] }>('/games')
      .then(res => setGames(res.items))
      .catch(() => {})
    api.get<{ gameId: string }[]>('/subscriptions')
      .then(subs => setSubscribedIds(new Set(subs.map(s => s.gameId))))
      .catch(() => {})
  }, [])
            ...
            filtered.map(g => (
              ...
              >
                <div className="min-w-0">
                  <Link href={`/adventures/${g.slug}`} className="font-semibold hover:underline">
                    {g.name}
                  </Link>
                  {g.description && (
