...
interface NewCharacterClientProps {
  initialGames: MyGame[]
  initialSchemas: CharacterSchema[]
  initialGameId: string | null
}

export function NewCharacterClient({ initialGames, initialSchemas, initialGameId }: NewCharacterClientProps) {
  const { user } = useAuth()
  const router = useRouter()

  const activeGames = initialGames.filter(g => g.status === 'active' && (g.role === 'owner' || g.role === 'gm'))
  const preselectedGame = activeGames.find(g => g.id === initialGameId) ?? null

  const [step, setStep] = useState<Step>(preselectedGame ? 'race' : 'adventure')
  const [selectedGameId, setSelectedGameId] = useState(preselectedGame?.id ?? '')
  const [raceSchemas, setRaceSchemas] = useState<CharacterSchema[]>(
    preselectedGame ? initialSchemas.filter(s => s.isActive && s.type === 'race') : [],
  )
  const [classSchemas, setClassSchemas] = useState<CharacterSchema[]>(
    preselectedGame ? initialSchemas.filter(s => s.isActive && s.type === 'class') : [],
  )
